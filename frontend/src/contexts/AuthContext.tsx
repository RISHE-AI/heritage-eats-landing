...
import { applyTheme, getSavedTheme, saveTheme } from "@/config/themes";

const THEME_STORAGE_KEY = "maghizam-theme";

/** Save the current theme under a user-specific key, then restore it on next login */
const saveUserTheme = (userId: string) => {
  const current = getSavedTheme();
  localStorage.setItem(`${THEME_STORAGE_KEY}-${userId}`, current);
};

/** Restore user's preferred theme after login */
const restoreUserTheme = (userId: string) => {
  const userTheme = localStorage.getItem(`${THEME_STORAGE_KEY}-${userId}`);
  if (userTheme) {
    saveTheme(userTheme);
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const logout = useCallback(() => {
    // Save current user's theme preference before clearing
    if (user?.id) {
      saveUserTheme(user.id);
    }
    setUser(null);
    localStorage.removeItem(AUTH_TOKEN_KEY);
    localStorage.removeItem(AUTH_USER_KEY);
    // Reset theme to default light-red immediately
