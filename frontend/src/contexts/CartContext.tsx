...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Reusable: reload cart from backend and update state
  const reloadCartFromBackend = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to reload cart from backend:', err);
    }
  }, [user]);
  ...
  // Listen for chatbot cart updates and reload instantly
  useEffect(() => {
    const handler = () => {
      reloadCartFromBackend();
    };
    window.addEventListener('chatbot-cart-updated', handler);
    return () => window.removeEventListener('chatbot-cart-updated', handler);
  }, [reloadCartFromBackend]);
