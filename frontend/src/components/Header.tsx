...
import { ShoppingCart, Menu, User, Search, X, ChevronRight, Package, Building2 } from "lucide-react";
...
const NAV_LINKS = [
  { href: "#top", labelEn: "Home", labelTa: "முகப்பு" },
      ...
      >
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-14 md:h-16">
            {/* Logo */}
            <Link to="/" className="flex items-center shrink-0 group" aria-label="Maghizam Home">
              <img 
                src="/maghizam-logo.png" 
                alt="Maghizam Foods Logo" 
                className="h-10 w-10 md:h-12 md:w-12 object-cover rounded-full shadow-sm group-hover:shadow-md transition-all duration-300 group-hover:scale-105" 
              />
            </Link>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                <SheetContent side="right" className="w-80 p-0">
                  ...
                  <div className="flex flex-col h-full">
                    {/* Mobile Header */}
                    <div className="p-5 border-b bg-gradient-to-br from-primary/5 to-transparent flex justify-center items-center">
                      <img 
                        src="/maghizam-logo.png" 
                        alt="Maghizam Foods Logo" 
                    ...
                    {/* Extra Links (Mobile) */}
                    <div className="px-4 mt-4 mb-2 space-y-1">
                      <Link to="/track" onClick={() => setMobileMenuOpen(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium hover:bg-primary/5 hover:text-primary transition-colors text-foreground">
                        <Package className="h-4 w-4 text-muted-foreground" /> Track Order
                      </Link>
                      <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium hover:bg-primary/5 hover:text-primary transition-colors text-foreground">
                        <Building2 className="h-4 w-4 text-muted-foreground" /> About Us
                      </Link>
                    </div>
