    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] md:max-w-3xl lg:max-w-4xl max-h-[92vh] overflow-y-auto p-0 rounded-2xl">
        ...
        <div className="grid gap-0 md:grid-cols-2">
          {/* ── Image Carousel ── */}
          <div className="bg-secondary/10 md:rounded-l-2xl flex flex-col justify-center overflow-hidden relative p-3">
            <ImageCarousel
        ...
        {/* Product Details */}
        <div className="border-t px-4 md:px-6 pt-5 pb-4 space-y-3">

              {/* Product Description Card */}
              {(product.descriptionEn || product.descriptionTa) && (
                <div className="rounded-2xl border border-blue-200/40 dark:border-blue-900/40 bg-blue-50/60 dark:bg-blue-950/20 overflow-hidden hover:shadow-md transition-shadow duration-200">
                  <div className="flex items-center gap-2 px-4 py-3 border-b border-blue-200/40 dark:border-blue-900/40">
                    <div className="w-7 h-7 rounded-lg bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center">
                      <FileText className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-blue-800 dark:text-blue-300">Product Description</h4>
                      <p className="text-[10px] text-blue-600/70 dark:text-blue-500 tamil-text">தயாரிப்பு விவரம்</p>
                    </div>
                  </div>
                  <div className="px-4 py-3 space-y-2">
                    {product.descriptionEn && (
                      <p className="text-xs text-foreground/80 leading-relaxed">{product.descriptionEn}</p>
                    )}
                    {product.descriptionTa && (
                      <p className="text-xs text-foreground/70 leading-relaxed tamil-text">{product.descriptionTa}</p>
                    )}
                  </div>
                </div>
              )}
