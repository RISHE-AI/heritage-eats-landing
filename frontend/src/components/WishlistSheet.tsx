...
const WishlistSheet: React.FC = () => {
  ...
  const handleProductClick = (product: any) => {
    setOpen(false);
    // Dispatch custom event so Index.tsx can open ProductModal
    window.dispatchEvent(new CustomEvent("wishlist-product-click", { detail: product }));
  };
            ...
            items.map((product) => (
              ...
              >
                <img
                  ...
                  alt={product.nameEn}
                  className="h-20 w-20 rounded-lg object-cover bg-secondary cursor-pointer hover:opacity-80 transition-opacity"
